...
class DialogEditSensor extends React.Component {
  ...
  saveChangings = () => {
    ...
    this.props.data.map((sensor) => {
      if(sensor.chipId === this.props.chipId) {
        dataNew.push({ chipId: this.props.chipId, chip_id: this.props.chipId, color: this.state.color, fav: this.props.fav, name: this.state.name });
      } else {
        dataNew.push(sensor);
      }
      return true;
  ...
  render() {
    ...
    return (
      <Dialog open={this.props.opened} onClose={this.props.onClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" >
        <DialogTitle id="alert-dialog-title">{strings.editSensor}</DialogTitle>
        <div className={classes.dialogContent}>
          <p><TextField label={strings.displayName} value={this.state.name} style={{width: "320px"}} onChange={this.nameChanged} variant="outlined"/></p>
          <p><TextField label={strings.chipId} value={this.props.chipId} style={{width: "320px"}} variant="outlined" disabled/></p>
